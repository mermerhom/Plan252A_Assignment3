        ...
        for i in range(present_trips.shape[-2]):
            for j in range(growth_rate.shape[-1]):
                afm_base_growth_rate[(present_trips.shape[-2] * (t + 1)) + i, j] = new_recharging_growth_rate[
                    i, j]  # advanced
    mock_convergence = np.ones((present_trips.shape[-2], afm_base_growth_rate.shape[-1]), dtype=float)
    afm_convergence = afm_base_growth_rate[
                      (present_trips.shape[-2] * afm_iteration): present_trips.shape[-2] * (afm_iteration + 1),
                      0: afm_base_growth_rate.shape[-1]]
...
def dm(present_trips, future_trips, growth_rate, dm_iteration):
    ...
    dm_convergence = dm_base_growth_rate[(present_trips.shape[-2] * dm_iteration):
...
def fm(present_trips, future_trips, growth_rate, fm_iteration):
    ...
    fm_convergence = fm_base_growth_rate[(present_trips.shape[-2] * fm_iteration):
