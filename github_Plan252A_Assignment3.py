...
def afm(present_trips, future_trips, growth_rate, afm_iteration):
    ...
    afm_base_growth_rate = np.zeros((growth_rate.shape[-2] * (afm_iteration + 1), growth_rate.shape[-1]))
    afm_array = np.zeros(((afm_iteration + 1) * present_trips.shape[-2], present_trips.shape[-1]))
    ...
    afm_convergence = afm_growth_rate[
                      ((present_trips.shape[-2] * afm_iteration) - present_trips.shape[-2]): present_trips.shape[
                                                                                                 -2] * afm_iteration,
...
def dm(present_trips, future_trips, growth_rate, dm_iteration):
    ...
    future_trips = future_trips
    growth_rate = growth_rate
    dm_base_growth_rate = np.zeros((growth_rate.shape[-2] * (dm_iteration + 1), growth_rate.shape[-1]))
    dm_array = np.zeros(((dm_iteration + 1) * present_trips.shape[-2], present_trips.shape[-1]))
    ...
    dm_convergence = dm_base_growth_rate[((present_trips.shape[-2] * dm_iteration) - present_trips.shape[-2]):
                                         present_trips.shape[-2] * dm_iteration, 0: dm_base_growth_rate.shape[-1]]
...
def fm(present_trips, future_trips, growth_rate, fm_iteration):
    ...
    future_trips = future_trips
    growth_rate = growth_rate
    fm_base_growth_rate = np.zeros((growth_rate.shape[-2] * (fm_iteration + 1), growth_rate.shape[-1]))
    fm_array = np.zeros(((fm_iteration + 1) * present_trips.shape[-2], present_trips.shape[-1]))
    ...
    mock_convergence = np.ones_like(present_trips, dtype=float)
    fm_convergence = fm_growth_rate[((present_trips.shape[-2] * fm_iteration) - present_trips.shape[-2]): present_trips.shape[-2] * fm_iteration, 0: present_trips.shape[-1]]
...
comparison_df_plot.set_title("Trip Distribution Methods Comparison\nNumber of Zones = 5, 10 and 15, Initial Mean of Trips = 480,"
...
comparison_1_df_plot_legend = comparison_df_plot.legend(labelspacing=1, edgecolor = "#1874CD", fontsize=10, frameon=False, bbox_to_anchor=(0.8, 0.52),
...
comparison_df_plot_assignment.set_title("Trip Distribution Methods Comparison\nData: Assignment",
                                        fontdict={"font": "Nirmala UI", "color": "black", "fontsize": 10, "weight": "bold"})
